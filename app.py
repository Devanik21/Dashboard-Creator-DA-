...
"""
        ...
        # NEW FEATURE 27: Cohort Analysis
        with st.expander("👥 Cohort Analysis"):
            st.subheader("Analyze User Behavior Over Time by Cohorts")
            if not df.empty and date_cols:
                st.write("Cohort analysis helps understand how groups of users (cohorts) behave over time. Cohorts are typically defined by their acquisition date.")

                user_id_col_cohort = st.selectbox("Select User/Customer ID Column", categorical_cols + numeric_cols, key="cohort_user_id")
                order_date_col_cohort = st.selectbox("Select Order/Event Date Column", date_cols, key="cohort_order_date")
                # metric_col_cohort = st.selectbox("Select Metric Column (e.g., Revenue, Activity Count)", numeric_cols, key="cohort_metric") # Optional for more advanced cohort metrics

                if user_id_col_cohort and order_date_col_cohort:
                    if st.button("Generate Cohort Retention Matrix", key="generate_cohort_button"):
                        try:
                            cohort_df = df[[user_id_col_cohort, order_date_col_cohort]].copy().dropna()
                            cohort_df['OrderMonth'] = cohort_df[order_date_col_cohort].dt.to_period('M')
                            cohort_df['CohortMonth'] = cohort_df.groupby(user_id_col_cohort)['OrderMonth'].transform('min')
                            
                            def get_cohort_period(df_cohort_calc, event_month_col, cohort_month_col):
                                df_cohort_calc['CohortPeriod'] = (df_cohort_calc[event_month_col].dt.year - df_cohort_calc[cohort_month_col].dt.year) * 12 + \
                                                              (df_cohort_calc[event_month_col].dt.month - df_cohort_calc[cohort_month_col].dt.month)
                                return df_cohort_calc

                            cohort_df = get_cohort_period(cohort_df, cohort_df['OrderMonth'], cohort_df['CohortMonth'])

                            cohort_pivot = cohort_df.groupby(['CohortMonth', 'CohortPeriod'])[user_id_col_cohort].nunique().reset_index()
                            cohort_counts = cohort_pivot.pivot_table(index='CohortMonth', columns='CohortPeriod', values=user_id_col_cohort)

                            cohort_sizes = cohort_counts.iloc[:, 0]
                            retention_matrix = cohort_counts.divide(cohort_sizes, axis=0)
                            retention_matrix = retention_matrix.applymap(lambda x: f"{x:.1%}" if pd.notnull(x) else '')

                            st.write("#### User Retention by Cohort (%)")
                            st.write("Each row represents a cohort (users acquired in that month). Columns represent months since acquisition. Values are the percentage of users from that cohort who were active in that subsequent month.")
                            
                            # For better display, convert PeriodIndex to string for Streamlit dataframe
                            retention_matrix_display = retention_matrix.copy()
                            retention_matrix_display.index = retention_matrix_display.index.strftime('%Y-%m')
                            st.dataframe(retention_matrix_display)

                            # Heatmap visualization
                            # Convert percentages back to float for heatmap
                            retention_matrix_heatmap = retention_matrix.applymap(lambda x: float(x.strip('%'))/100 if isinstance(x, str) and x.endswith('%') else np.nan)
                            retention_matrix_heatmap.index = retention_matrix_heatmap.index.strftime('%Y-%m') # For display

                            fig_cohort_heatmap = px.imshow(retention_matrix_heatmap, 
                                                           text_auto=".0%", 
                                                           aspect="auto",
                                                           labels=dict(x="Months Since Acquisition", y="Cohort Month", color="Retention Rate"),
                                                           title="Cohort Retention Heatmap")
                            fig_cohort_heatmap.update_xaxes(side="top")
                            st.plotly_chart(fig_cohort_heatmap, use_container_width=True)

                        except Exception as e:
                            st.error(f"Error generating cohort analysis: {e}")
                else:
                    st.info("Please select a User ID column and an Order/Event Date column.")
            elif not date_cols:
                st.info("Cohort analysis requires at least one date column in your dataset.")
            else:
                st.info("Upload data to perform cohort analysis.")

        # NEW FEATURE 28: Advanced Time Series Decomposition (STL)
        with st.expander("🕰️ Advanced Time Series Decomposition (STL)"):
            st.subheader("Decompose Time Series into Trend, Seasonal, and Residual Components")
            if date_cols and numeric_cols:
                from statsmodels.tsa.seasonal import STL # Local import

                ts_decomp_date_col = st.selectbox("Select Date Column for Decomposition", date_cols, key="ts_decomp_date")
                ts_decomp_value_col = st.selectbox("Select Value Column for Decomposition", numeric_cols, key="ts_decomp_value")
                ts_decomp_period = st.number_input("Seasonal Period (e.g., 7 for daily data with weekly seasonality, 12 for monthly with yearly)", min_value=2, value=12, key="ts_decomp_period")

                if ts_decomp_date_col and ts_decomp_value_col:
                    if st.button("Decompose Time Series", key="decompose_ts_button"):
                        try:
                            ts_data_decomp = df[[ts_decomp_date_col, ts_decomp_value_col]].copy().dropna()
                            ts_data_decomp = ts_data_decomp.sort_values(ts_decomp_date_col)
                            ts_data_decomp = ts_data_decomp.set_index(ts_decomp_date_col)[ts_decomp_value_col]
                            
                            # Resample to ensure regular frequency if needed, or let STL handle it if possible
                            # For simplicity, assuming data is somewhat regular or user has preprocessed.
                            # STL requires the period to be less than half the length of the series.
                            if len(ts_data_decomp) > 2 * ts_decomp_period:
                                stl = STL(ts_data_decomp, period=ts_decomp_period, robust=True)
                                result_stl = stl.fit()

                                fig_stl = result_stl.plot()
                                fig_stl.set_size_inches(10, 8) # Adjust plot size
                                st.pyplot(fig_stl)
                            else:
                                st.warning(f"Not enough data for the chosen seasonal period. Need at least {2 * ts_decomp_period + 1} data points. You have {len(ts_data_decomp)}.")
                        except Exception as e:
                            st.error(f"Error during STL decomposition: {e}")
            else:
                st.info("Time series decomposition requires at least one date column and one numeric column.")
